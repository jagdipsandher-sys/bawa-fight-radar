#!/usr/bin/env python3
"""
Rebuilds the Raiders tab of index.html from ESPN's NRL fixture feed, so the
draw maintains itself instead of being hand-typed once and left to rot.

Structurally this is the Panthers builder again, pointed at a different club.
The one real difference: Panthers currently sell every 2026 home ticket
through Ticketmaster only (a stadium-redevelopment quirk), so that builder
branches ticket vendor by home/away. The Raiders have no such arrangement —
their games (home and away) run through Ticketek's general NRL portal — so
this version uses a single ticket link throughout. Ladder position is not
hand-typed anywhere here: it moves week to week and this script has no live
standings feed, so the season card links out to the real ladder instead of
asserting a number that could go stale.
...
Only the blocks between the BUILD:RAI- markers are touched. The season summary
...
def title(g):
    return f"Raiders v {g['opponent']}" if g["home"] else f"{g['opponent']} v Raiders"
...
def note(g):
    ...
    return "Home game at GIO Stadium"
...
def row(g):
    ...
    return f"""      <tr{' class="big"' if home else ''} data-sport="nrl" data-ends="{ends(g)}" data-card="{cid(g)}">
        ...
        <td>{esc(g['venue'])}</td>
        <td><a class="mini buy" href="{TICKETS}">Tickets · Ticketek</a><button class="mini" onclick="openCard('{cid(g)}')">Details</button><button class="mini" onclick="addCal('{cid(g)}')">+ Cal</button></td>
      </tr>"""
...
def hero(g):
    ...
    return f"""    <div class="card" data-slot="nrl" data-ends="{ends(g)}">
      ...
      <div class="crest dark">
        <div class="big">Raiders <em>v</em> {esc(g['opponent'])}</div>
      ...
      <div class="inner">
        ...
        <div class="meta">{esc(g['venue'])}</div>
        <ul class="mc">
          <li>{'Home game at GIO Stadium, Canberra.' if home else 'Away from home — check the venue’s own ticketing before planning a trip.'}</li>
        </ul>
        <div class="btns">
          <a class="btn red" href="{TICKETS}">Tickets</a>
...
def card(g):
    ...
    bouts = [
        ...
        ["Tickets", "Ticketek — the NRL's ticketing partner for this fixture"],
    ]
...
def finals_tail(games):
    ...
    return """
      <tr data-sport="nrl" data-ends="2026-09-28T23:59:00+10:00">
        ...
        <td>TBC — depends on the ladder</td>
...
STATIC_CARDS = """  raiFinals: {
    ...
    secs:[
      ...
      {h:"What It Means For Canberra",b:[
        ["A top-eight finish keeps the season alive","Miss the eight and the year ends after Round 27",1],
        ["A top-four finish means a second chance","Lose a qualifying final and you still get another week"],
        ["Fixtures appear on this tab automatically","They are pulled from the NRL feed as soon as the draw is published"]
      ]}
    ],
    note:"Nothing about the finals is bookable until the ladder locks in. Check the live NRL ladder for where the Raiders currently sit."
  },
  raiGF: {
    emoji:"🏆",
...
def main():
    games = collect()
    if not games:
        sys.exit("no Raiders fixtures returned — leaving the tab as it is")
    ...
    page = open(PAGE).read()
    before = page
    page = splice(page, "RAI-HERO", hero(games[0]))
    page = splice(page, "RAI-ROWS", "\n".join(row(g) for g in games) + finals_tail(games))
    page = splice(page, "RAI-CARDS", ",\n".join([card(g) for g in games] + [STATIC_CARDS]))

    if page == before:
        print("raiders tab already current")
        return
    with open(PAGE, "w") as f:
        f.write(page)
    print(f"rebuilt the Raiders tab: {len(games)} fixtures")
